...
func SearchPlayer(c *gin.Context) {
	...
	if err := db.Unscoped().Where("CAST(display_name as CHAR(20) CHARACTER SET latin1) LIKE ?", "%"+name+"%").Limit(limit).Offset((page - 1) * limit).Find(&users).Error; err != nil {
...
func SearchIP(c *gin.Context) {
	...
	q := db.Unscoped().Model(&models.Connection{}).Joins("User").Where("remote_addr LIKE ? or local_addr LIKE ?", ip+"%", ip+"%")
